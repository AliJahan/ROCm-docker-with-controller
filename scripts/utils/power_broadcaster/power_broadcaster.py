...
def broadcast_power(publish_socket, cpu_read_interval_msec, gpu_read_interval_msec, broad_cast_interval_sec):
    ...
    while True:
        ...
        if gpu_power is None:
            ...
            break
        print(f"Got gpu ({gpu_mon.get_time_str()})! ", end="", flush=True)
        cpu_power = cpu_mon.get_avg_power()
        print(f"Got cpu ({gpu_mon.get_time_str()})! ", end="", flush=True)
        ...
        try:
            ...
            break
        print(f"published ({gpu_mon.get_time_str()})! ", flush=True)
...
def main():
    cpu_read_interval_msec = 1000
    gpu_read_interval_msec = 100
